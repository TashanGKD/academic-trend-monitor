...
- `frontend/`：静态站点前端
- `pipeline/` + `config/` + `tests/`：离线数据处理流水线，相当于这里的“后端代码”

这个仓库不包含以下生成内容：

- `data/raw/` 下的原始 arXiv 数据
- `data/output/`、`data/recent.jsonl`、`data/weekly/`、`data/analysis/` 下的流水线产物
- `frontend/public/data/` 和 `frontend/dist/` 下的部署拷贝与构建产物

首次拉取后，如需完整运行，需要自行准备数据并执行流水线或部署命令生成这些文件。
